...
def get_vendors_orders(request, vendor_id):
    ...
    try:
        ...
        return APIResponse(f"No orders found for vendor {vendor_id}", status.HTTP_200_OK, data=[])
    except Exception as e:
        print(e)
        logging.error(e)
        return APIResponse("Internal Error.", status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def get_order_items(request, vendor_id, po_id):
    ...
    try:
        ...
        if orders:
            ...
            return APIResponse(f"Order with ID {po_id} not found for vendor {internal_id}",
